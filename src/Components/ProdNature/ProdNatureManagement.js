...
import ConfirmDeleteModal from "./ProdDeleteNature";
import EditEmployeeModal from "./ProdUpdateNature";
import AddNature from "./ProdAddNature";  // Make sure this import is correct
import axiosInstance from '../Utils/axiosInstance'
import { toast, ToastContainer } from 'react-toastify';

const ProdNatureManagement = ({ handleOpenModal }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axiosInstance.get("/ProductionNature"); // Update with your API
    ...
    <div className="container mt-4">
      ...
      <div className="row align-items-center mb-4">
        <div className="col text-center">
          <h5 className="mb-0">Production Nature Management</h5>
        </div>
          ...
          {loading ? (
            ...
            data.map((nature, index) => (
              <tr key={nature.id || index}>
                ...
                <td>{nature.productionType}</td>
                <td>{nature.manpower}</td>
                <td>{nature.norms}</td>
                {/* <td>{new Date(nature.startDate).toLocaleDateString('en-GB')}</td> */}
                <td>
                  <button
...
export default ProdNatureManagement;
