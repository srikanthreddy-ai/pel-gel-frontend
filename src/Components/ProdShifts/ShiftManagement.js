...
import ConfirmDeleteModal from "./ProdDeleteShifts";
import EditEmployeeModal from "./ProdUpdateShifts";
import AddNature from "./ProdAddShifts"; // Make sure this import is correct
...
const ProdShifts = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axiosInstance.get("/ProductionShift"); // Update with your API
    ...
    <div className="container mt-4">
      ...
      <div className="row align-items-center mb-4">
        <div className="col text-center">
          <h5 className="mb-0">Production Shifts</h5>
        </div>
          ...
          {loading ? (
            ...
            data.map((nature, index) => (
              <tr key={nature.id || index} style={nature.isDeleted ? { backgroundColor: '#ffe6e6', color: '#a00' } : {}}>
                <td>{index + 1}</td>
                <td>{nature.shiftName}</td>
                <td>{nature.shiftHrs}</td>
                <td>{nature.startTime}</td>
                <td>{nature.endTime}</td>
                <td>
                  <button
